...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (partnerForm) {
    partnerForm.addEventListener('submit', (e) => {
      ...
      const subject = encodeURIComponent('Nieuwe partner aanvraag: ' + (data.venueName || ''));
      const body = encodeURIComponent(
        'Naam locatie: ' + (data.venueName || '') + '\n' +
        'Contactpersoon: ' + (data.contactPerson || '') + '\n' +
        'Telefoonnummer: ' + (data.contactPhone || '') + '\n' +
        'Email: ' + (data.contactEmail || '') + '\n' +
        'Locatie / Stad: ' + (data.venueLocation || '') + '\n' +
        'Type locatie: ' + (data.venueType || '')
      );
      window.open('mailto:nerocharge@outlook.com?subject=' + subject + '&body=' + body, '_self');
